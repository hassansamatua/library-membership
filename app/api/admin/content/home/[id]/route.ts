...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    // Get specific home content item
    const [homeContent] = await connection.query<RowDataPacket[]>(
      'SELECT id, section_key, section_type, content, order_index, is_active, created_at, updated_at FROM home_content WHERE id = ?',
      [id]
    );
    ...
    return NextResponse.json(homeContent[0]);
  } catch (error) {
