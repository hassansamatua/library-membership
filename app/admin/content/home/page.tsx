...
export default function AdminHomePage() {
  ...
  const fetchHomeContent = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/admin/content/home', { credentials: 'include' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      let url = '/api/admin/content/home';
      let method = 'POST';
      
      if (isEditing) {
        url = `/api/admin/content/home/${selectedItem.id}`;
        method = 'PUT';
      }
      ...
      if (!response.ok) throw new Error(`Failed to ${isEditing ? 'update' : 'create'} home content`);

      toast.success(`Home content ${isEditing ? 'updated' : 'created'} successfully`);
      setShowModal(false);
      resetForm();
      fetchHomeContent();
    } catch (error) {
      console.error('Error saving home content:', error);
      toast.error(`Failed to ${isEditing ? 'update' : 'create'} home content`);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      setHomeContent(homeContent.filter(p => p.id !== id));
  ...
  const getFilteredData = () => {
    return homeContent.filter(item =>
  ...
  const getSectionIcon = (type: string) => {
    switch (type) {
      case 'heading': return FiHome;
      case 'subheading': return FiBook;
      case 'text': return FiBook;
      case 'list_item': return FiCheck;
      case 'image': return FiCalendar;
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        <h1 className="text-2xl font-semibold text-gray-900">Home Content Management</h1>
        <p className="text-gray-600">Manage homepage content sections</p>
      </div>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-4 text-center text-gray-500">
                    {searchTerm ? 'No content found matching your search' : 'No home content found'}
                  </td>
                </tr>
              ) : (
                getFilteredData().map((item: HomeContent) => {
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900">
                  {isEditing ? 'Edit Home Content' : 'Add Home Content'}
                </h2>
                <button
                  onClick={() => {
                    setShowModal(false);
