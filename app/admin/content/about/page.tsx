...
export default function AdminAboutPage() {
  ...
  const fetchAboutContent = async () => {
    try {
      setIsLoading(true);
      const response = await fetch('/api/admin/content/about', { credentials: 'include' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      let url = '/api/admin/content/about';
      let method = 'POST';
      
      if (isEditing) {
        url = `/api/admin/content/about/${selectedItem.id}`;
        method = 'PUT';
      }
      ...
      if (!response.ok) throw new Error(`Failed to ${isEditing ? 'update' : 'create'} about content`);
      ...
      toast.error(`Failed to ${isEditing ? 'update' : 'create'} about content`);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`/api/admin/content/about/${id}`, {
        method: 'DELETE',
        credentials: 'include',
      });
      if (!response.ok) throw new Error('Failed to delete content');

      setAboutContent(aboutContent.filter(p => p.id !== id));
  ...
  const getSectionIcon = (type: string) => {
    switch (type) {
      case 'heading': return FiInfo;
      case 'subheading': return FiInfo;
      case 'text': return FiInfo;
      case 'list_item': return FiCheck;
      case 'image': return FiEye;
      case 'mission': return FiTarget;
      case 'vision': return FiEye;
      case 'values': return FiHeart;
  ...
  const getSectionTypeColor = (type: string) => {
    switch (type) {
      ...
      case 'values': return 'bg-pink-100 text-pink-800';
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-4 text-center text-gray-500">
                    {searchTerm ? 'No content found matching your search' : 'No about content found'}
                  </td>
                </tr>
              ) : (
                getFilteredData().map((item: AboutContent) => {
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-2xl font-bold text-gray-900">
                  {isEditing ? 'Edit About Content' : 'Add About Content'}
                </h2>
                <button
                  onClick={() => {
                    setShowModal(false);
